---
title: "gbHumanitarian coverage and boundary metadata"
description: |
  gbHumanitarian boundaries are mirrored from UN OCHA. Check current metadata.
knitr:
  opts_chunk:
    collapse: true
    comment: "#>"
...
These boundaries are not covered by the package's MIT license. Check the current
boundary metadata for the original source, license and attribution terms.

```{r}
#| label: ghuman-coverage
#| echo: false
#| fig-cap: "Highest available ADM level by country: gbHumanitarian."
library(geobounds)
library(ggplot2)
library(dplyr)
...
ggplot(p) +
  ...
  labs(
    ...
    caption = "Source: geoBoundaries (CGAZ and gbHumanitarian metadata)"
  )
```

## `r src` metadata

The table lists boundary metadata and archive links for **gbHumanitarian**. Use
