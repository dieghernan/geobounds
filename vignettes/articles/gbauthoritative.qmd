---
title: "gbAuthoritative coverage and boundary metadata"
description: |
...
These UN SALB data are not covered by the package's MIT License and are
restricted to non-commercial use. Review the [SALB terms of
use](https://salb.un.org/sites/default/files/wysiwyg_uploads/docs_uploads/TermsOfUseSALB2021.pdf)
before reuse.

```{r}
#| label: gautho-coverage
#| echo: false
#| fig-cap: "Highest available ADM level by country: gbAuthoritative."
library(geobounds)
library(ggplot2)
library(dplyr)
...
ggplot(p) +
  ...
  labs(
    fill = "Highest available ADM level",
    caption = "Source: geoBoundaries (CGAZ and gbAuthoritative metadata)"
  )
```

## `r src` metadata

The table lists boundary metadata and download links for **gbAuthoritative**.
Use the filters or search box to find a country or boundary type.
