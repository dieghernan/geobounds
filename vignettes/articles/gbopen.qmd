---
title: "gbOpen coverage and boundary metadata"
description: |
  gbOpen provides openly licensed boundaries with licenses reported in metadata.
knitr:
  opts_chunk:
    collapse: true
    comment: "#>"
...
The map shows the highest ADM level available for each country in the **gbOpen**
release type.

The original license varies by boundary and may include ODbL, CC BY, CC BY-SA or
national open-data terms. Check the current metadata before reuse.

```{r}
#| label: map-coverage
#| echo: false
#| fig-cap: "Highest available ADM level by country: gbOpen."
library(geobounds)
library(ggplot2)
library(dplyr)
...
The table lists boundary metadata and download links for **gbOpen**. Use the
filters or search box to find a country or boundary type.
