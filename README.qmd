...
the [**geoBoundaries**](https://www.geoboundaries.org/) Global Database of
Political Administrative Boundaries. Using this package, you can:

- Programmatically retrieve individual country files and global composite files
  from **geoBoundaries**.
- Use **tidyverse** and **sf** workflows in **R** to map, analyze and join
  administrative boundary data with your own data.
...
simplifies downloads, caching and integration with spatial data workflows.
...
```{r}
#| label: simple_plot
#| fig-cap: Map of ADM1, ADM2 and ADM3 boundaries for Sri Lanka
library(geobounds)
...
geoBoundaries provides three release types with different sources, validation
...
- **gbAuthoritative**: Boundaries mirrored from UN SALB and verified through
  in-country processes. These boundaries cannot be used for commercial
  purposes.
...
attribution is provided. Other release types may have additional licensing
limits, so check the metadata returned by `gb_get_metadata()` before reuse.
